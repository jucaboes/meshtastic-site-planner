<!-- Colombia fork: etiquetas en espanol; rango prefijado a 30 km en store.ts -->
<template>
  <div>
    <p class="mt-hint mb-3">Confianza estadistica y radio maximo a calcular. Rangos mayores demoran mas.</p>
    <div class="grid grid-cols-2 gap-2">
      <div>
        <label for="situation_fraction" class="mt-label">Fraccion de situacion (%)</label>
        <input v-model="simulation.situation_fraction" type="number" class="mt-input" id="situation_fraction" min="1" max="100" step="0.1" />
      </div>
      <div>
        <label for="time_fraction" class="mt-label">Fraccion de tiempo (%)</label>
        <input v-model="simulation.time_fraction" type="number" class="mt-input" id="time_fraction" min="1" max="100" step="0.1" />
      </div>
      <div class="col-span-2">
        <label for="simulation_extent" class="mt-label">Radio maximo (km)</label>
    ...
    <label class="mt-3 flex cursor-pointer items-center gap-3">
      ...
      <span class="text-sm font-medium text-ink">Terreno alta resolucion (30 m)</span>
    </label>
    <p class="mt-hint mt-1">9x mas detalle que la cuadricula de 90 m por defecto. Mas lento y limitado a 70 km.</p>
  </div>
</template>
